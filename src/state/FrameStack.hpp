...
public:
    ...
    FrameStack(char* string_ptr, int string_area_size, int global_area_size) {
        ...
        __gc_stack_bottom = stack_data + max_stack;
        ...
        __gc_stack_top = __gc_stack_bottom - string_size - global_area_size;
    }
    ...
    void push_stack_frame(const uint32_t nargs, const char* ra, const uint32_t isClosure = 0) {
        ...
        *(__gc_stack_top - 1) = reinterpret_cast<uint64_t>(fp);
        *(__gc_stack_top - 2) = reinterpret_cast<uint64_t>(__gc_stack_top + (nargs + isClosure));
